...
def compute_I_cone(p: ConeRadParams):
    ...
    if multi_I:
        ...
        for cone_idx, zmu_c in enumerate(p.z_mu_cone):
            ...
            for phi_ray_idx, phi in enumerate(p.phi_nodes):
                if is_prom:
                    mu_out = np.cos(phi) * sinTheta #Projection of the muOut from the y-x plane to the x axis in the prominence case
                else:
                    mu_out = zmu_c #Raw zmu in the filament case

                mu_in = outgoing_chromo_ray_mu(mu_out, p.alt) #Find the input ray from the chromosphere based on this muOut
                mu_ins.append(mu_in)
    ...
    for cone_idx, zmu_c in enumerate(p.z_mu_cone):
        ...
        for phi_ray_idx, phi in enumerate(p.phi_nodes):
            ...
            mu_in = outgoing_chromo_ray_mu(mu_out, p.alt) #Find the input ray from the chromosphere based on this muOut
            Icone += p.provider.compute_I(p.wavelength, mu_in) * p.w_phi_nodes[phi_ray_idx]

        Iin += Icone * p.w_mu_cone[cone_idx] #Once the cone has been summed, multiply it by the cone weight from the original GL quadrature set within the prominence model
    return Iin
    ...
    def __init__(self, projection: str, bc_provider: PromBcProvider, altitude_m: float,
        ...
        if prom_upper_lower is not None and projection == "prominence":
            if prom_upper_lower not in ["upper", "lower"]:
                raise ValueError(f"Expected prom_upper_lower ({prom_upper_lower}), to be 'upper' or 'lower'")
    ...
    def update_bc(self, atmos, spect):
        ...
        for mu_idx in range(Nmu):
            if final_synth:
                ...
                lower_mu = 0.0 if mu_idx == 0 else 0.5 * (self.muz[mu_idx - 1] + self.muz[mu_idx])
                upper_mu = 1.0 if mu_idx == self.muz.shape[0]-1 else 0.5 * (self.muz[mu_idx] + self.muz[mu_idx + 1])
                cone_half_width = 0.5 * (upper_mu - lower_mu)
                cone_mid = 0.5 * (lower_mu + upper_mu)
                cone_zmu = cone_half_width * self.x_cone + cone_mid #xCone only represents x sampling for order NconeRays GL quadrature - then shifted based on bounds of cone
                cone_wmu = self.w_cone #Weights correspond to the quadrature order

                # NOTE(jmj): Compute the intensity integrated within the cone
                params = ConeRadParams(spect.wavelength, cone_zmu, cone_wmu, phi_nodes, w_phi_nodes, self.altitude,
                                       self.projection, self.provider)
