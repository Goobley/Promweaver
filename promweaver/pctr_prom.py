...
class PctrPromModel(PromModel):
    ...
    def __init__(
        ...
        pressure_half = (
            4.0 * pc * cmass_half / max_cmass * (1.0 - cmass_half / max_cmass)
            + self.tr_pressure
        )
        temperature_half = (
            self.cen_temperature
            + (self.tr_temperature - self.cen_temperature)
            * (1.0 - 4.0 * cmass_half / max_cmass * (1.0 - cmass_half / max_cmass))
            ** gamma
        )
        self.pressure = np.concatenate([pressure_half, pressure_half[:-1][::-1]])
        self.temperature = np.concatenate(
            [temperature_half, temperature_half[:-1][::-1]]
        )
        self.cmass = cmass
        ...
        for k in range(rho.shape[0]):
            ...
            ne[k] = eos.pe_from_pg(self.temperature[k], pressure_cgs[k]) / (
                witt.BK * self.temperature[k]
            )

        rho = (rho << u.Unit("g / cm3")).to("kg / m3").value
        ...
        if add_extra_rays is not None:
            ...
            atmos.rays(
                muz=np.concatenate([atmos.muz, add_extra_rays["muz"]]),
                mux=np.concatenate([atmos.mux, add_extra_rays["mux"]]),
                wmu=np.concatenate([atmos.wmu, extra_wmu])
            )
